...
void main() {
  ...
  float alpha = clamp(dot(vertexNormal, normalize(ToLightVector + ToCameraVector)), 0, 1);
  float theta = clamp(dot(vertexNormal, ToLightVector), 0, 1);

  // compute ambient component
  vec4 ambient = ka*Ia;
  // compute diffuse component
  vec4 diffuse = kd*Id*theta;
  // compute specular component
  vec4 specular = ks*Is*pow(alpha, specExp);
